...
namespace ModManager {
    public abstract class ModButton {
        ...
        protected List<Dependency> RelevantIssues => _relevantIssues ??= Requirements.Where(i => i.Severity >= SeverityThreshold).ToList();
        ...
        internal virtual void DoModIssuesIcon(Rect canvas) {
            ...
            Dependency worst = Requirements.MaxBy( d => d.Severity );
        ...
        internal virtual void DrawRequirements(ref Rect canvas) {
            int severityThreshold = ModManager.Settings.ShowSatisfiedRequirements ? 0 : 1;
            IEnumerable<Dependency> relevantIssues = Requirements.Where( i => i.Severity >= severityThreshold );
            if (!relevantIssues.Any()) {
                return;
            }

            Utilities.DoLabel(ref canvas, I18n.Dependencies);
            Rect outRect = new Rect(canvas) { height = (relevantIssues.Count() * LineHeight) + (SmallMargin * 2f) };
            ...
            foreach (Dependency issue in relevantIssues) {
                Rect iconRect = new Rect(issueRect.xMin, issueRect.yMin, SmallIconSize, SmallIconSize)
                    .CenteredOnYIn(issueRect);
                Rect labelRect = new Rect(issueRect);
