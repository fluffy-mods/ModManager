...
namespace ModManager {
    public class ModButton_Installed: ModButton {
        ...
        public static ModButton_Installed For(ModMetaData mod) {
            ModButton_Installed button = ModButtonManager.AllButtons.OfType<ModButton_Installed>()
        ...
        public override Color Color {
            get {
                ...
                if (!Lists.NullOrEmpty()) {
                    IEnumerable<Color> colours = Lists.Select(l => l.Color)
        ...
        internal virtual void DoSourceButtons(Rect canvas) {
            ...
            bool singleVersion = VersionsOrdered.Count() == 1;
            foreach (ModMetaData mod in VersionsOrdered) {
                Texture2D icon = mod.Source.GetIcon();
                Color color = mod.VersionCompatible ? Color.white : Color.red;
                GUI.color = color;
                if (singleVersion) {
        ...
        public void DoModActionFloatMenu() {
            ...
            options.Add(new FloatMenuOption(I18n.ChangeColour, () => {
                List<FloatMenuOption> options2 = NewOptionsList;
        ...
        internal override void DoModDetails(Rect canvas) {
            ModMetaData mod = Selected;
            if (!mod.PreviewImage.NullOrBad()) {
                DoLabel(ref canvas, I18n.Preview);
                int width = mod.PreviewImage.width;
                int height = mod.PreviewImage.height;
                float scale = canvas.width / width;
            ...
            Rect authorRect = new Rect(
                ...
                (detailRect.width - SmallMargin) / 2f,
                LineHeight);
            Rect targetVersionRect = new Rect(
                titleRect.xMax,
                detailRect.yMin,
                (detailRect.width - SmallMargin) / 2f,
                LineHeight);
            ...
            // title
            float labelWidth = MaxWidth(I18n.Title, I18n.Author);
            Text.Font = GameFont.Small;
            Text.Anchor = TextAnchor.UpperLeft;
            ...
            if (!mod.AuthorsString.NullOrEmpty()) {
                ...
                ModMetaData steamMod = mod.Source switch {
                    ContentSource.ModsFolder => mod.UserData()?.Source,
                    ContentSource.SteamWorkshop => mod,
                    ContentSource.Undefined => throw new NotImplementedException(),
                    ContentSource.OfficialModsFolder => throw new NotImplementedException(),
                    _ => null,
                };
                if (steamMod != null && SteamManager.Initialized) {
                    CSteamID authorId = Traverse.Create(steamMod.GetWorkshopItemHook())
                        .Field("steamAuthor")
                        .GetValue<CSteamID>();
            ...
            Widgets.DrawBoxSolid(canvas, SlightlyDarkBackground);
            Rect descriptionOutRect = canvas.ContractedBy(SmallMargin).Rounded();

            // description
            float height2 = Text.CalcHeight(mod.Description, descriptionOutRect.width);
            Rect descriptionViewRect = new Rect(
