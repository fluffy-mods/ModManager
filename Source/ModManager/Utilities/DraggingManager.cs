...
namespace ModManager {
    public static class DraggingManager {
        ...
        public static bool ContainerUpdate<T>(IEnumerable<T> mods, Rect rect, out int index) where T : ModButton {
            ...
            float position = ( Event.current.mousePosition.y - rect.yMin ) / ModButtonHeight;

            // start drag
            int dragIndex = Mathf.FloorToInt( position );
            int clampedDragIndex = Mathf.Clamp( dragIndex, 0, mods.Count() - 1 );
            ...
            if (Dragging) {
                int dropIndex = Mathf.RoundToInt( position );
                int clampedDropIndex = Mathf.Clamp( dropIndex, 0, mods.Count() );
        ...
        public static void OnGUI() {
            ...
            Rect rect = new Rect( 0, 0, ModButtonWidth, ModButtonHeight );
